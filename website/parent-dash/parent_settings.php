...
function sp_parent_remove_profile_image_file(?string $path): void
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_profile') {
        ...
        if (!empty($_POST['remove_profile_image'])) {
            sp_parent_remove_profile_image_file($newImagePath);
            $newImagePath = null;
        }
        ...
        if ($fn !== '' && $profileError === '') {
            $pdo->prepare("UPDATE users SET first_name=:fn, last_name=:ln, phone=:ph, profile_image=:img WHERE user_id=:uid")
                ->execute(['fn'=>$fn,'ln'=>$ln,'ph'=>$ph?:null,'img'=>$newImagePath,'uid'=>$userId]);
...
$initial = strtoupper(substr(trim((string)($profile['first_name'] ?? 'P')), 0, 1));
...
<section class="sp-card">
  <div class="sp-card__hd"><div><div class="sp-card__title">Profile Settings</div><div class="sp-card__sub">Photo, contact and account details</div></div></div>
...
<section class="sp-split sp-split--gap-lg">
  ...
  <section class="sp-card">
    <div class="sp-card__hd"><div><div class="sp-card__title">Payment Details</div><div class="sp-card__sub">Visual preview only</div></div></div>
    <div class="sp-card__bd">
      <div class="sp-stack sp-stack--sm">
        <div class="sp-statline"><span>Cardholder</span><strong><?php echo htmlspecialchars(trim((string)($profile['first_name'] ?? '') . ' ' . (string)($profile['last_name'] ?? ''))); ?></strong></div>
        <div class="sp-statline"><span>Card</span><strong>**** **** **** 3169</strong></div>
        <div class="sp-statline"><span>Expiry</span><strong>11/29</strong></div>
        <div class="sp-statline"><span>Billing ZIP</span><strong>14623</strong></div>
      </div>
      <div class="sp-settings-grid" style="margin-top:12px;">
        <div class="sp-settings-field"><label class="sp-form-label">Card number</label><input class="sp-input" style="width:100%;" placeholder="5555 5555 5555 4444"></div>
        <div class="sp-settings-field"><label class="sp-form-label">Expiry</label><input class="sp-input" style="width:100%;" placeholder="MM/YY"></div>
        <div class="sp-settings-field"><label class="sp-form-label">CVC</label><input class="sp-input" style="width:100%;" placeholder="123"></div>
      </div>
      <div class="sp-form-actions" style="margin-top:14px;">
        <button class="sp-btn sp-btn--ghost" type="button">Update Payment (Preview)</button>
      </div>
    </div>
  </section>
</section>
