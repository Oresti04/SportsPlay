...
<section class="sp-page-grid">
  <section class="sp-card sp-page-hero">
    <div class="sp-hero">
      <div>
        ...
        <p>Coach dashboard &middot; overview with roster, schedule, messages and announcements.</p>
      </div>
  ...
  <section class="sp-card sp-surface">
    <div class="sp-card__hd">
      ...
      <span class="sp-pill" style="background:rgba(255,255,255,.14); color:#fff; border-color:rgba(255,255,255,.2);"><i class="fa-solid fa-shield-halved"></i> <?php echo htmlspecialchars($coach['team']['sport']); ?></span>
    </div>
...
<section class="sp-split sp-split--gap-lg">
  <section class="sp-card">
    <div class="sp-card__hd">
      <div><div class="sp-card__title">Upcoming Schedule</div><div class="sp-card__sub">Trainings + matches from database</div></div>
      <a class="sp-btn sp-btn--ghost" href="coach_team.php#schedule"><i class="fa-solid fa-calendar-days"></i> Full Schedule</a>
    </div>
    <div class="sp-card__bd">
      <?php if (empty($coach['schedule'])): ?>
        <p class="sp-muted">No upcoming events scheduled.</p>
      <?php else: ?>
        <div class="sp-stack">
          <?php foreach (array_slice($coach['schedule'], 0, 4) as $s): ?>
  ...
  <section class="sp-card">
    <div class="sp-card__hd">
      <div><div class="sp-card__title">Announcements</div><div class="sp-card__sub">Team broadcasts</div></div>
      <a class="sp-btn sp-btn--primary" href="coach_announcements.php"><i class="fa-solid fa-bullhorn"></i> Manage</a>
    </div>
    <div class="sp-card__bd">
      <?php if (empty($coach['announcements'])): ?>
        <p class="sp-muted">No announcements yet. Create one to notify your team.</p>
      <?php else: ?>
        <div class="sp-stack">
          <?php foreach (array_slice($coach['announcements'], 0, 3) as $a): ?>
            <article class="sp-announcement-card">
              <h3><?php echo htmlspecialchars($a['title']); ?></h3>
              <div class="sp-announcement-card__sub"><?php echo htmlspecialchars($a['time']); ?><?php if (!empty($a['audience'])): ?> &middot; <?php echo htmlspecialchars(ucfirst($a['audience'])); ?><?php endif; ?></div>
